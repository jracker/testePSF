...
Na próxima etapa aplicaremos o PSF a todas as partições do conjunto de  treinamento e armazenaremos quais valores dos parâmetros `k` e `w` foram utilizados.
...
A avaliação do desempenho do modelo foi feita a partir da média do desempenho, nesse caso baseado na métrica KGE, do PSF aplicado a cada partição dos dados de treinamento.
...
Selecionaremos os valores ótimos a partir da média dos parâmetros `k` e `w`.
...
Utilizando 15 divisões nos dados de treinamento e a  média dos parâmetros `k` e `w` foi possível observar, baseado na métrica KGE, uma pequena melhora no modelo.
